    ...
    async readChunk(options){
        ...
        for(let i = 0; i < tweets.length; i++){
            ...
            if(tweets[i - 1]){
                tweets[i].links[0] = {
                    rel: 'prev',
                    href: `http://localhost:3000/api/v1/users/${tweets[i].authorId}/tweets/` + tweets[i - 1].id
                };
            }
            tweets[i].links[1] = {
                rel: 'self',
                href: `http://localhost:3000/api/v1/users/${tweets[i].authorId}/tweets/` + tweets[i].id
            };
            if(tweets[i + 1]){
                tweets[i].links[2] = {
                    rel: 'next',
